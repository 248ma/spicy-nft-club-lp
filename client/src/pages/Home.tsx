    ...
    <div className="min-h-screen bg-background text-foreground font-sans">
      ...
      <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <img 
            src="/images/hero-bg-cyberpunk.jpg" 
            alt="Cyberpunk Lounge" 
        ...
        <div className="container relative z-10">
          ...
          <div className="relative">
            <motion.div 
              ...
              transition={{ duration: 0.8 }}
            >
              <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">{t('features_section.title')}</h2>
              <div className="w-20 h-1 bg-gradient-to-r from-[#ff0080] to-[#7928ca] mx-auto rounded-full" />
            </motion.div>
            ...
            >
              {(t('features_section.items', { returnObjects: true }) as any[]).map((item, i) => {
                ...
                return (
                  <motion.div key={i} variants={staggerItem} className="h-full">
                    <div className={`group relative h-full bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-4 md:p-6 hover:bg-white/5 transition-all duration-300 ${borderColors[i]} flex flex-col items-start`}>
                      
                      {/* Header: Icon + Number */}
                      <div className="flex items-center justify-between w-full mb-3 md:mb-4">
                        <div className={`w-10 h-10 md:w-12 md:h-12 rounded-lg bg-gradient-to-br ${gradients[i]} flex items-center justify-center border border-white/10 group-hover:scale-110 transition-transform duration-300`}>
                          {icons[i]}
                        </div>
                        <div className="text-xs md:text-sm font-mono text-gray-600 group-hover:text-gray-400 transition-colors">0{i + 1}</div>
                      </div>

                      {/* Content */}
                      <div className="mt-auto w-full">
                        <h3 className="text-sm md:text-lg font-bold text-white mb-2 leading-tight min-h-[2.5em] md:min-h-[auto] flex items-center">
                          {item.title}
                        </h3>
                        <p className="text-[10px] md:text-sm text-gray-400 leading-relaxed line-clamp-3 md:line-clamp-none">
                          {item.description}
                        </p>
                      </div>
                      ...
                      <div className="absolute top-0 right-0 w-8 h-8 overflow-hidden rounded-tr-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <div className={`absolute top-0 right-0 w-12 h-12 bg-gradient-to-bl ${gradients[i]} blur-md opacity-40`} />
                      </div>
                    </div>
                  </motion.div>
                );
        ...
        <div className="container relative z-10">
          <div className="grid md:grid-cols-2 gap-20 items-center max-w-7xl mx-auto">
            <motion.div {...fadeInUp} className="order-2 md:order-1 text-center md:text-left">
              ...
              <h2 className="text-5xl md:text-6xl font-bold mb-8 text-white leading-tight">{t('solution.title')}</h2>
              <p className="text-xl text-gray-400 mb-12 leading-relaxed border-l-4 border-primary/50 pl-6 md:text-left text-left mx-auto md:mx-0">
              ...
              <div className="space-y-6">
                {(t('solution.items', { returnObjects: true }) as any[]).map((item, i) => (
                  <motion.div 
                    key={i}
                    className="glass-panel p-6 rounded-xl flex flex-col md:flex-row gap-6 hover:bg-white/5 transition-colors items-center md:items-start text-center md:text-left"
                    initial={{ opacity: 0, x: -20 }}
                    whileInView={{ opacity: 1, x: 0 }}
            ...
            >
              <div className="relative w-full max-w-lg aspect-[3/4] perspective-1000">
                ...
                <div className="flex flex-col items-center mt-6">
                  <span className="text-[10px] text-gray-400 uppercase tracking-widest mb-1">{t('hero.current_price')}</span>
                  <p className="text-center text-sm text-white/80 font-mono tracking-wider drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]">
                    ETH: #001 / 0.01eth
                  </p>
                </div>
              </div>
